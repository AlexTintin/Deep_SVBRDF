...
class DUnet(nn.Module):
    def __init__(self):
        ...
        self.unmawpool = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        self.flat = Flatten()
        self.lin = nn.Linear(512*2*2, 512*2*2*2)
        self.unflat = UnFlatten()
#        self.premodel = Resfeat(device)
    ...
    def decode(self,x_appearence,x15,x13,x11,x9,x7,x5,x3,x1):
        x_appearence = self.unflat(x_appearence)
        xm1 = self.up0(torch.cat([x_appearence, x15], dim=1))
    ...
    def forward(self, x,y):
        ...
        x = self.decode(z,xlatent,x13, x11,x9,x7,x5,x3,x1)
        return x, mu,logvar
