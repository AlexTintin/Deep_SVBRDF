...
class AEG(nn.Module):
    def __init__(self):
        ...
        self.encoder0 = nn.Conv2d(10, 64, kernel_size=4,padding=1, stride=(2, 2))
        torch.nn.init.normal_(self.encoder0)
        self.encoder1 = nn.Conv2d(64, 128, kernel_size=4, padding=1, stride=(2, 2))
        torch.nn.init.normal_(self.encoder1)
        self.encoder2 = nn.Conv2d(128, 256, kernel_size=4, padding=1, stride=(2, 2))
        torch.nn.init.normal_(self.encoder2)
        self.encoder3 = nn.Conv2d(256, 512, kernel_size=4, padding=1, stride=(2, 2))
        torch.nn.init.normal_(self.encoder3)
        self.encoder4 = nn.Conv2d(512, 512, kernel_size=4, padding=1, stride=(2, 2))
        torch.nn.init.normal_(self.encoder4)
        self.lrelu = nn.LeakyReLU(0.2,True)
        self.batch = nn.BatchNorm2d(512)
        self.tanh = nn.Tanh()

        self.decoder0 = nn.Conv2d(512, 512, kernel_size=4, padding=1, stride=(2, 2))
        torch.nn.init.normal_(self.encoder0)
        self.decoder1 = nn.Conv2d(512, 256, kernel_size=4, padding=1, stride=(2, 2))
        torch.nn.init.normal_(self.encoder1)
        self.decoder2 = nn.Conv2d(256, 128, kernel_size=4, padding=1, stride=(2, 2))
        torch.nn.init.normal_(self.encoder2)
        self.decoder3 = nn.Conv2d(128, 64, kernel_size=4, padding=1, stride=(2, 2))
        torch.nn.init.normal_(self.encoder3)
        self.decoder4 = nn.Conv2d(64, 10, kernel_size=4, padding=1, stride=(2, 2))
        torch.nn.init.normal_(self.encoder4)
